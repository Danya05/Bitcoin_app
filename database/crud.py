...
@db_session
def create_transaction(sender: User, amount_btc_without_fee: float,
                       receiver_address: str, fee: float | None = None, testnet: bool = False):
    """
    ...
    """
    wallet_of_sender = bit.Key(sender.wallet.private_key) if not testnet\
        else bit.PrivateKeyTestnet(sender.wallet.private_key)

    sender.wallet.balance = int(wallet_of_sender.get_balance())
...
def get_transaction_info(transaction: pydantic_models.Transaction):
    return {
        ...
        "sender_wallet": transaction.wallet if transaction.wallet else None,
